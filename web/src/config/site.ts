...
export const site = {
  ...
  analytics: {
    ...
    metaPixelId: "1036291125518805",  // Meta Pixel id (live 2026-06-23) — HARDCODED, not in GTM
    // Google Tag Manager container (marketing-owned). Owns Google Ads conversions
    // (and any future GA4 / remarketing) — those are NOT hardcoded. track() pushes
    // our semantic events (whatsapp_click/lead_submit/brochure_request) to the
    // dataLayer so GTM triggers can fire conversions off them. Empty = off (dev).
    // OWNERSHIP RULE: GTM must NOT re-add Meta Pixel or Clarity — code owns those.
    gtmId: "GTM-5FCBHXLJ",
  },
} as const;
